<template>
  <div class="jishubowen">
    <ul class="BlogUl">
      <li class="liItem" :key="index" v-for="(item,index) in AllArticle" @click="GotoArticleDetail(item.id,item.clickNum)">
        <div class="img">
          <img v-lazy="item.picUrl" alt="">
        </div>
        <div class="title">{{item.diseaseName}}</div>
        <div class="brief">{{item.diseaseExplanation}}</div>
        <div class="tag">
...
<!--          <el-tag v-if="item.class_name03" type="info">{{ item.class_name03}}</el-tag>-->
        ...
        <div class="Item-end">
          <span>+文章阅读</span>
          <span class="timer">{{item.createTime | timer()}}</span>
